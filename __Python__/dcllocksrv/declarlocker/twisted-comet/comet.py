...
from: http://twisted-comet.googlecode.com
'''
import urlparse
import logging

from twisted.web import server, resource, http, error, xmlrpc
...
class Subject(resource.Resource):
    ...
    def notify(self, data):
        ...
        """
        log.info('Subject %s notified %d observers with data "%s"',
            self.name, len(self.observers), data)
        self.callId.cancel()
        for observer in self.observers:
            observer.notify(data)
        self.callback(self)
