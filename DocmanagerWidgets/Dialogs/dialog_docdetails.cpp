...
MFCDocumentInfo * Dialog_DocDetails::exec( const QStringList &doctypes, const QStringList &docagencies, const QString &num, QDate date, QDate expires )
{
  ...
  ui->cBox_Docagency->setEditable( true );
  ...
  currentIndex = -1;
  if ( ui->cBox_Docagency->count() == 1 ) currentIndex = 0;
  ui->cBox_Docagency->setCurrentIndex( currentIndex );
...
int Dialog_DocDetails::exec( MFCDocumentInfo *doc, WriteMode writeMode )
{
  ...
  ui->cBox_Docagency->setEditable( writeMode != WritePagesnum &&
                                   writeMode != ReadOnly );
  ...
  ui->cBox_Docagency->clear();
  ui->cBox_Docagency->addItem( doc->agency() );
