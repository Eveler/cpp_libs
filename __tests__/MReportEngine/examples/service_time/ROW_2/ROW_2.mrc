<report>
	<parameters>
		<parameter name="$P_ATTACH_VISIBLE_USERNAME_2$">
			<parameter_type name="Repeater"/>
			<parameter_source name="$P_MAIN_VISIBLE_USERNAME_2$"/>
		</parameter>
		<parameter name="$P_ATTACH_FIRSTDATE_2$">
			<parameter_type name="Foreign parameter"/>
			<parameter_source name="$P_MAIN_FIRSTDATE$"/>
		</parameter>
		<parameter name="$P_ATTACH_LASTDATE_2$">
			<parameter_type name="Foreign parameter"/>
	...
	WHERE q_1."Время обслуживания" IS NOT NULL AND "Дата/время обращения">='$P_ATTACH_FIRSTDATE_2$' AND "Дата/время обращения"<='$P_ATTACH_LASTDATE_2$'
		AND "Специалист принявший обращение"='$P_ATTACH_VISIBLE_USERNAME_2$'
	...
	ORDER BY "Специалист принявший обращение", "Дата/время обращения"::date, "Время обслуживания") q_2]]>
		...
		<key name="$COL_5$">
			...
			<key_data_source><![CDATA[SELECT COUNT(*) AS dp_cnt FROM docpaths_prelim_view dpw
				WHERE "Дата/время обращения"::date>='$P_ATTACH_FIRSTDATE_2$'::date
					AND "Дата/время обращения"::date<='$P_ATTACH_LASTDATE_2$'::date
					AND "Специалист принявший обращение"='$P_ATTACH_VISIBLE_USERNAME_2$'
					AND appeal_type=1
	...
	FROM (SELECT "Время обслуживания", "Специалист принявший обращение", "Дата/время обращения"::date AS "Дата обращения"
		...
		WHERE "Время обслуживания" IS NOT NULL AND "Дата/время обращения">='$P_ATTACH_FIRSTDATE_2$'
			AND "Дата/время обращения"<='$P_ATTACH_LASTDATE_2$'
			AND "Специалист принявший обращение"='$P_ATTACH_VISIBLE_USERNAME_2$') q_2
	...
	FROM (SELECT COUNT(*) AS dp_cnt FROM docpaths_prelim_view dpw
		WHERE "Дата/время обращения"::date>='$P_ATTACH_FIRSTDATE_2$'::date
			AND "Дата/время обращения"::date<='$P_ATTACH_LASTDATE_2$'::date
			AND "Специалист принявший обращение"='$P_ATTACH_VISIBLE_USERNAME_2$') dp_q,
	(SELECT count(*) AS cons_cnt
		...
		WHERE "Время обслуживания" IS NOT NULL AND "Дата/время обращения"::date>='$P_ATTACH_FIRSTDATE_2$'::date
			AND "Дата/время обращения"::date<='$P_ATTACH_LASTDATE_2$'::date
			AND "Специалист принявший обращение"='$P_ATTACH_VISIBLE_USERNAME_2$') cons_q]]>
			</key_data_source>
		</key>
		<key name="$COL_10$">
			<key_source_type name="SQL with parameters"/>
	...
	FROM (SELECT (dp_q.dp_cnt+cons_q.cons_cnt) AS cnt
			...
			(SELECT count(*) AS cons_cnt
				...
				WHERE "Время обслуживания" IS NOT NULL AND "Дата/время обращения"::date>='$P_ATTACH_FIRSTDATE_2$'::date
					AND "Дата/время обращения"::date<='$P_ATTACH_LASTDATE_2$'::date
					AND "Специалист принявший обращение"='$P_ATTACH_VISIBLE_USERNAME_2$') cons_q) cnt_all,
