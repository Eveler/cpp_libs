...
public:
    ...
    static bool parseDir(const QByteArray &buffer, const QString &userName, FileInfo *info);

signals:
    void listInfo(const FileInfo&);
    void readyRead();
...
bool QFtpDTP::parseDir(const QByteArray &buffer, const QString &userName, FileInfo *info)
{
    if (buffer.isEmpty())
        return false;
...
//        QString line = QString::fromAscii(commandSocket.readLine());
      QString line = QVariant( commandSocket.readLine() ).toString();
...
//            line = QString::fromAscii(commandSocket.readLine());
            line = QVariant( commandSocket.readLine() ).toString();
    ...
    : QObject(*new QFtpPrivate, parent)
{
    ...
    connect(&d->pi.dtp, SIGNAL(listInfo(FileInfo)),
            SIGNAL(listInfo(FileInfo)));
}
...
/*!
    \fn void QFtp::listInfo(const FileInfo &i);
